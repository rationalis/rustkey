...
impl State {
    pub fn matcher(
        ...
        move |e| {
            let key_match = match (key.as_ref(), e.usb_keycode()) {
                (None, _) => true,
                (Some(ks), k) => ks.contains(&k),
            };

            let released_match = match (released, e.keyup_time) {
                (None, _) | (Some(true), Some(_)) | (Some(false), None) => true,
                _ => false,
            };

            let pressed_before_match = match pressed_before {
                None => true,
                Some(t) => e.keydown_time() <= t,
            };

            let extends_past_match = match (extends_past, e.keyup_time) {
                (None, _) => true,
                (Some(t), Some(t2)) => t2 >= t,
                (Some(_), None) => false, // TODO: should compare against now
            };

            key_match && released_match && pressed_before_match && extends_past_match
        }
    }

    pub fn push(&mut self, key: UsbKeycode, ev_type: EventType, time: SystemTime) {
        match ev_type {
            EventType::KeyDown => {
            ...
            EventType::KeyUp => {
                let ev = self
                    ...
                    .iter_mut()
                    .rev()
                    .find(Self::matcher(Some(vec![key]), Some(false), None, None))
                    .unwrap();
